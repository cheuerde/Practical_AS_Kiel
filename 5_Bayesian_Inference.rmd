...
In statistical modelling, the Bayes theorem inferes the probability of observing the parameters of interest,
...
of our data (likelihood) and the prior distribution of our parameters we wish to estimate.
...
virtually any prior distribution for any parameter can be chosen, regardless of the likelihood distribution.
...
The probability of our parameters given the data ($p(\mu|y)$) is proportional to the product of the likelihood of the data given our parameter ($p(y|\theta)$,
which is a normal density function, and the prior likelihood of the parameter ($p(\theta)$), which is a normal density function again, which mean
...
Bayesian Inference allows us to incorporate actualy prior evidence in the estimation
